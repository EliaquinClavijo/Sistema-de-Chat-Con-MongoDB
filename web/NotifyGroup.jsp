...
<%    } else {
        ...
        haySesion =  (String)sesionOk.getAttribute("haySesion");
                ...
                DBCollection miColeccion = baseDatos.getCollection("GrNotify");
                ...
                if (G.get("usuario").toString().equals(Host.toString())){
                    if(G.get("visto").toString().compareTo("0") == 0){
                                %>
                                <script>
                                    Notificate("<%=G.get("host").toString()+" a " +G.get("grupo").toString()%>", "<%=G.get("mensaje").toString()%>");
                                </script>
                                <%
                                //Actualizar la base
